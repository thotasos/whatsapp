...
function init() {
  if (_db) return;  // already initialized — prevent double-init connection leak
  fs.mkdirSync(WA_DIR, { recursive: true });
  _db = new Database(path.join(WA_DIR, 'messages.db'));
  _db.pragma('journal_mode = WAL');

  // Add media_path column if it doesn't exist (migration)
  try { _db.exec('ALTER TABLE messages ADD COLUMN media_path TEXT'); } catch {}
...
function insertMessage(msg) {
  _db.prepare(`
    INSERT OR IGNORE INTO messages
      (timestamp, sender_jid, sender_name, chat_jid, chat_name, text, media_path, is_group, is_from_me, wa_message_id)
    VALUES
      (@timestamp, @sender_jid, @sender_name, @chat_jid, @chat_name, @text, @media_path, @is_group, @is_from_me, @wa_message_id)
  `).run({
