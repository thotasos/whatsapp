...
async function sendMedia(jid, buffer, mimeType, caption, fileName) {
  if (!sock || _state !== 'connected') throw new Error('Not connected to WhatsApp');
  const mediaType = mimeType.startsWith('image/') ? 'image' :
                    mimeType.startsWith('video/') ? 'video' :
                    mimeType.startsWith('audio/') ? 'audio' : 'document';

  // Build message content - fileName only applies to documents
  const msgContent = { caption: caption || undefined, mimetype: mimeType };
  if (mediaType === 'document' && fileName) {
    msgContent.document = buffer;
    msgContent.fileName = fileName;
  } else {
    msgContent[mediaType] = buffer;
  }
